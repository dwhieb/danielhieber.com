...
/**
 * Authentication middleware for passing to individual routes.
...
const authenticate = (req, res, next) => {
  ...
  db.createDocument(db.coll, authReq, (err, doc) => {
...
const oauthCallback = (req, res, next) => {
  ...
  const getAuthReq = () => new Promise((resolve, reject) => {
    const docLink = `${db.coll}/docs/${req.query.state}`;
  ...
  passport.authenticate('windowslive', (err, user) => {
