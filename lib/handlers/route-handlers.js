...
exports.cv = (req, res) => {
  ...
  db.queryDocuments(db.coll, query).toArray((err, results) => {
    ...
    res.locals.publication.sort((a, b) => a.date < b.date);
    ...
    publications.forEach(pub => {
      if (!(pub.publicationType in res.locals.publication)) {
        res.locals.publication[pub.publicationType] = [];
      }
      res.locals.publication[pub.publicationType].push(pub);
    });
