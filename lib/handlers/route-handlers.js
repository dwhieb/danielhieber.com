  ...
  db.queryDocuments(db.coll, query).toArray((err, results) => {
    ...
    res.locals.education.forEach(ed => {
      ed.achievements.forEach((ach, i) => {
        ed.achievements[i] = markdown.render(ach);
      });
    });

    // media
    res.locals.media.forEach(media => {
      const dates = getDateStrings(media.date);
      media.datetime = dates.datetime;
      media.dateString = dates.dateString;
    });
    ...
    publications.forEach(pub => {
      ...
      pub.title = markdown.render(pub.title);

      if (pub.date === 'forthcoming') {
        pub.datetime = '';
        pub.year = pub.date;
      } else {
        const dates = getDateStrings(pub.date);
        pub.datetime = dates.datetime;
        pub.year = dates.date.getFullYear();
      }
...
exports.languages = (req, res) => {
  ...
  if (req.params.language) {
    ...
    const render = references => {

      const title   = capitalize(langName);
      const context = { pageTitle: title };

      res.locals.nav.languages.selected = true;
      if (references) context.references = references;

      // the callback handles the case where the provided language name does not exist
      res.render(`languages/${langName}`, context, (err, html) => {
    ...
    if (langName === 'chitimacha') mendeley.getDocs().then(render).catch(render);
    else render();
