...
exports.cv = (req, res) => {
  ...
  db.queryDocuments(db.coll, query).toArray((err, results) => {
    ...
    // add the first CV item to the array for its type in res.locals
    res.locals[results[0].type].push(results[0]);

    /* eslint-disable no-underscore-dangle */

    // retrieve the most recently updated document
    // add each item to its proper array in res.locals in the process
    const lastUpdatedDocument = results.reduce((prev, current) => {
      res.locals[current.type].push(current);
      return current._ts > prev._ts ? current : prev;
    });

    // separate proficiencies into skills and proficiencies
    ...
    res.locals.proficiency.skill.sort((a, b) => a.title > b.title);
    res.locals.proficiency.software.sort((a, b) => a.title > b.title);
    res.locals.service.sort((a, b) => a.startYear < b.startYear);
    res.locals.work.sort((a, b) => a.startYear < b.startYear);

    // create a Date object from the timestamp of the most recently updated document
    const lastUpdatedDate = new Date(lastUpdatedDocument._ts * 1000);

    /* eslint-enable no-underscore-dangle */

    res.locals.header = false;
    res.locals.nav.cv.selected = true;
    res.locals.lastUpdatedISO = lastUpdatedDate.toISOString().slice(0, 10);
    res.locals.lastUpdatedString = lastUpdatedDate.toLocaleDateString('en-US', {
      year:  'numeric',
      month: 'long',
      day:   'numeric',
    });
