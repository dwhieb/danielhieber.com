...
module.exports = socket => {
  ...
  return {
    ...
    addCategory(data, cb) {
      ...
      if (category) {
        db.createDocument(config.collLink, category, (err, res) => {
          if (err) return runCallback(cb, createError(err, category.id));
    ...
    deleteCategory(category, cb) {
      ...
      if (!category) {
        ...
        const err = {
          ...
          details: 'A category must be provided for deletion.',
        };

        sendResponse(err, null, cb);
