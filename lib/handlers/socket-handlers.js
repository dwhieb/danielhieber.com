...
module.exports = socket => {
  ...
  /**
   * Deletes a blob
   * @param  {String}   filename The name of the blog to delete
   * @param  {Function} cb       The callback to run when complete
   */
  const deleteFile = (filename, cb) => {
    storage.deleteBlob('publications', filename, (err, res) => sendResponse(err, res, cb));
  };
  ...
  const upsertFile = (filename, file, cb) => {

    if (typeof filename !== 'string') {
      const err = new JsonError(400, 'The first argument passed to "upsertFile" the socket must be a string containing the name of the file to upload.');
      return sendResponse(err, null, cb);
    }

    if (!Buffer.isBuffer(file)) {
      const err = new JsonError(400, 'The socket must be passed a Blob, ArrayBuffer, or File.');
      return sendResponse(err, null, cb);
    }
    ...
    storage.createBlockBlobFromStream(
      'publications',
      filename,
      stream,
      file.length,
      (err, res) => {
        if (err) return sendResponse(err, null, cb);
        sendResponse(null, { status: 200, details: 'File uploaded successfully.' }, cb);
      });
