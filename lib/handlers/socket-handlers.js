...
// checks whether a type attribute is valid
const validType = type => type && type in models;
...
module.exports = socket => {
  ...
  // send the list of files in the /gallery folder to the client on connection
  const connect = () => socket.emit('gallery', images);
  ...
  const add = (data = {}, cb) => {
    ...
    if (validType(data.type)) {
      ...
      try {
        model = new models[data.type];
      } catch (err) {
        runCallback(cb, new JsonError(400, err.message));
        return;
      }

      db.createDocument(config.collLink, model, (err, res) => {
    ...
    const jsonError = new JsonError(400, `The data to add must include a valid "type" attribute.`);
  ...
  const get = (docs, cb) => {
    ...
    if (haveIds) {
      ...
      .then(() => sendResponse(Array.isArray(docs) ? results : results[0], results, cb))
      .catch(err => {
        const jsonError = convertError(err);
        sendResponse(jsonError, null, cb);
      });
  ...
  const update = (data = {}, cb) => {
    ...
    if (validType(data.type)) {
      ...
      db.upsertDocument(config.collLink, model, (err, res) => sendResponse(err, res, cb));

    }

    const jsonError = new JsonError(400, `The item to update must include a valid "type" attribute.`);
